    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          backgroundColor: '#FAF7FB', border: '1.5px solid #17171F', marginBottom: '14px',
        }}>
          <span style={{ fontSize: '12px', color: '#9C9CB1', fontWeight: 600 }}>Code:</span>
          <span style={{ fontSize: '16px', fontWeight: 800, color: '#17171F', letterSpacing: '2px', flex: 1 }}>
            {currentRoom.inviteCode}
          </span>
          <button onClick={handleCopyCode} style={{
            padding: '4px 12px', borderRadius: '8px',
            border: '1.5px solid #17171F', backgroundColor: copied ? '#64E0C6' : '#FAF7FB',
            fontSize: '12px', fontWeight: 600, cursor: 'pointer',
            color: copied ? '#fff' : '#9C9CB1',
          }}>
            {copied ? 'Copied!' : 'Copy'}
          </button>
        </div>
        ...
        {/* Config */}
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', fontSize: '13px', color: '#9C9CB1', marginBottom: '14px' }}>
        ...
        {/* Players */}
        <h4 style={{ fontSize: '13px', fontWeight: 700, color: '#9C9CB1', textTransform: 'uppercase', margin: '0 0 8px' }}>
          ...
          {currentRoom.players.map(p => (
            <div key={p.id} style={{
              ...
              <div style={{
                ...
                backgroundColor: p.color, border: '2px solid #17171F', flexShrink: 0,
              }} />
              <span style={{ fontWeight: 700, fontSize: '14px', color: '#17171F', flex: 1 }}>
                {p.name}
                {p.id === currentRoom.hostId && (
                  <span style={{ fontSize: '11px', color: '#FFD36B', marginLeft: '6px' }}>👑 Host</span>
                )}
                {p.isBot && (
                  <span style={{ fontSize: '11px', color: '#9C9CB1', marginLeft: '6px' }}>🤖 Bot</span>
                )}
              </span>
              <span style={{ fontSize: '12px', color: '#9C9CB1' }}>⭐ {p.rating}</span>
            </div>
          ))}
          {currentRoom.players.length < currentRoom.config.maxPlayers && (
            <div style={{
              padding: '10px 12px', borderRadius: '10px',
              backgroundColor: '#FAF7FB', border: '1.5px dashed #9C9CB1',
              textAlign: 'center', color: '#9C9CB1', fontSize: '13px', fontStyle: 'italic',
            }}>
              Waiting for players...
            </div>
          )}
      ...
      <div style={{
        ...
        border: '2px solid #17171F', backgroundColor: '#F3ECFF',
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: isMe ? '3px solid #17171F' : '2px solid transparent',
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center' }}>
        {isHost && (
          <button onClick={startRoom} disabled={!canStart} style={{
            ...
            backgroundColor: canStart ? '#64E0C6' : '#E0D6E6',
            color: '#fff', fontWeight: 700, fontSize: '16px',
            cursor: canStart ? 'pointer' : 'not-allowed',
            boxShadow: canStart ? '4px 4px 0 #17171F' : 'none',
        ...
        <button onClick={leaveRoom} style={{
          ...
          color: '#9C9CB1', fontWeight: 700, fontSize: '16px', cursor: 'pointer',
        }}>
          Leave Room
        </button>
      </div>
