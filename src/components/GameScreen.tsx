  ...
  return (
    <div className="game-layout" style={{
        ...
        <div style={sidebarCardStyle}>
          ...
          {config.players.map(player => (
            <div key={player.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              ...
              <span style={{ flex: 1, fontSize: '13px', fontWeight: 600, color: '#17171F' }}>{player.name}</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button
            ...
            style={{
              ...
              fontSize: '18px', color: '#9C9CB1', padding: '4px',
            }}
          >
            {muted ? '🔇' : '🔊'}
          </button>
      ...
      {phase === 'playing' && currentPlayer && (
        <div key={currentPlayerIndex} style={{
          padding: '10px 24px',
          borderRadius: '24px',
            ...
            {phase === 'matchEnd' && (
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
                  ...
                  <button onClick={() => setPhase('lobby')} style={actionBtnStyle('#FFD36B')}>Re-lobby</button>
          ...
          <div className="mobile-only" style={{
            ...
            {EMOTES.map(emote => (
              <button key={emote} onClick={() => handleEmote(emote)} style={{
                ...
                backgroundColor: '#FAF7FB', fontSize: '18px', cursor: 'pointer',
                lineHeight: 1,
              }}>
                {emote}
              </button>
      ...
      <aside className="game-sidebar">
        ...
        <div style={sidebarCardStyle}>
          <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#9C9CB1', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '12px' }}>Moves</h3>
          <div style={{ maxHeight: '200px', overflowY: 'auto', fontSize: '12px', color: '#444457' }}>
            {moveHistory.length === 0 ? (
              <p style={{ color: '#9C9CB1', fontStyle: 'italic' }}>No moves yet</p>
            ) : (
