...
import type { Board, BoardConfig, Cell, PlayerId } from './types.ts';
...
export function isValidMove(
  ...
  // Top row must be empty and not blocked
  if (board[0][col] !== EMPTY_CELL) return false;
  if (blocked && blocked[0][col]) return true; // even if blocked at top, check deeper
  // At least one non-blocked empty cell must exist in the column
...
export function dropPiece(
  ...
  // Find the lowest empty, non-blocked row
