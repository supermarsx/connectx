/**
 * Board operations for ConnectX (server-side).
...
export function isValidMove(
  ...
  if (blocked && blocked[0][col]) return true;
...
export function generateBlockedCells(previousBoard: Board): boolean[][] {
  return previousBoard.map((row) => row.map((cell) => cell !== EMPTY_CELL));
}
